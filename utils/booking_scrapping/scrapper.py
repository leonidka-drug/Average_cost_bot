...
class Scrapper:
    ...
    hotel_comments_class: str = "_4abc4c3d5 _1e6021d2f _6e869d6e0"
    ...
    def download_html(self) -> None:
        try:
            req = requests.get(self.url, headers=Scrapper.headers)
    ...
    def parse(self) -> tuple:
        if self.src != 0:
            ...
            self.ten_offers = self.soup.find_all(
                class_=Scrapper.hotel_cardboard_class
            )[0:10]

            for item in self.ten_offers:
                names.append(item.find(class_=Scrapper.hotel_name_class).text)
                urls.append(item.find(
                    "a",
                    class_=Scrapper.hotel_link_class
                ).get("href"))
                costs.append(int(item.find(
                    "span",
                    class_=Scrapper.hotel_room_cost_class
                ).text[0:-5].replace(" ", "")))
                comments.append(item.find(
                    class_=Scrapper.hotel_comments_class
                ).text)
