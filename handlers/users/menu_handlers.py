...
async def analyse_booking(message: Message, state: FSMContext) -> None:
    ...
    if url != None:
        ...
        info_without_URL = 'Чтобы проанализировать конкурентов, нужно отправить'
        info_without_URL += ' сюда URL booking.com сообщением. Для этого' 
        info_without_URL += ' войдите на сайт booking.com, выберите все'
        info_without_URL += ' соответсвующие поиску ваших конкурентов фильтры,'
        info_without_URL += ' которые можно видеть на фото в жёлтом окне'
        info_without_URL += ' "Найти" и под ним, и после загрузки страницы'
        info_without_URL += ' выделите весь URL-адрес как показано на фото,'
        info_without_URL += ' скопируйте и отправьте его сюда.'

        photo_path = BASE_DIR / "data/booking_screen.png"
        with photo_path.open("rb") as photo:
            await message.answer_photo(photo=photo,
                                       caption=info_without_URL,
